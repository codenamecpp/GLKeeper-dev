...
public:
    ...
    inline void SetMouseButtonState(unsigned button, bool state)
    {
        cxx_assert (button < GLFW_MOUSE_BUTTON_LAST);
    ...
    inline void SetKeyState(unsigned keycode, bool state)
    {
        cxx_assert(keycode < GLFW_KEY_LAST);
        mKeyboardKeys[keycode] = state;
    }

    // Get current key state
    // @param keycode: keycode
    inline bool GetKeyState(unsigned keycode) const 
    {
        cxx_assert(keycode < GLFW_KEY_LAST);
    ...
    // @param button: mouse button index
    inline bool GetMouseButton(unsigned button) const 
    {
        cxx_assert(button < GLFW_MOUSE_BUTTON_LAST);
